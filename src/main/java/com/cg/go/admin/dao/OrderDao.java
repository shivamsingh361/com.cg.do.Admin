...
public interface OrderDao extends CrudRepository<Order, String> {
	...
	List<ResultSet> quantityCancelledGroupByCategory(String date);
	 ...
	 @Query(value =" select p.category as Category, sum(opm.quantity*p.price) as Qty from order_table as ot inner join order_product_mapping opm using(order_id) "
	 ...
	 @Query(value = " select p.category as Category, sum(opm.quantity*p.price) as Qty from order_table as ot inner join order_product_mapping as opm using(order_id) "
